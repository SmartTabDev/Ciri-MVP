...
async def facebook_callback(
    ...
    if redirect_to == "settings":
        redirect_url = (
            ...
            f"&facebook_tokens={page.get('access_token', '')}"
        )
    else:
        redirect_url = (
...
async def instagram_callback(
    ...
    # NOW we await the async function
    useraccounts = await instagram_auth_service.get_instagram_user_accounts(access_token)
    if not useraccounts:
        return JSONResponse(
            {"error": "user_accounts_failed", "detail": "Could not fetch user accounts from instagram"},
            status_code=HTTP_400_BAD_REQUEST,
        )

    page = useraccounts['data'][0]
    instagram_page_id = page.get('id', '')
    instagram_page_access_token = page.get('access_token', '')
    name = page.get('name', '')

    userinfo = await instagram_auth_service.get_instagram_page_user_info(instagram_page_access_token, instagram_page_id)
    if not userinfo:
        return JSONResponse(
    ...
    instagram_user_id = userinfo.get("id", "") or ""
